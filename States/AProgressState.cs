...
{
    protected readonly Dictionary<TTaskId, TTask> Handlers = new();
    private Task? _task;
    ...
    public sealed override void Update()
    {
        ...
        AnsiConsole.Progress()
            .Start(pContext =>
            {
                Dictionary<TTaskId, ProgressTask> tasks = new();
    ...
    {
        ConsoleUtility.WriteMarkedLine(
            $"{GetType().Name} Task Error: {_task?.ToString() ?? "Task is NULL"}; Exception: {_task?.Exception?.ToString() ?? "NULL"}",
            EConsoleMark.Error);
        LogHandler.Error(DefaultErrorCode,
            $"{GetType().Name} Task Error: {_task?.ToString() ?? "Task is NULL"}; Exception: {_task?.Exception?.ToString() ?? "NULL"}");
